...
export default function CreateOrder() {
    ...
    const handleImageChange = async (e) => {
        ...
        if (images.length >= MAX_IMAGES_PER_ORDER) {
            alert(`Maximum ${MAX_IMAGES_PER_ORDER} images are allowed per order.`);
        ...
        for (const file of filesToProcess) {
            ...
            if (file.size > MAX_IMAGE_SIZE_BYTES) {
                alert(`File ${file.name} is too large (>10MB). Skipping.`);
                continue;
            }
            ...
            try {
                ...
                alert(`Failed to compress ${file.name}. Uploading original.`);
        ...
        if (files.length > filesToProcess.length) {
            alert(`Only ${MAX_IMAGES_PER_ORDER} images can be attached per order.`);
        }
